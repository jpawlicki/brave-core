...
void GettingStartedHelper::GetEducationURL(GetEducationURLCallback callback) {
  ...
  // Attempt to fetch the website URL in the background.
  ...
  url_loader_->DownloadToString(
      ...
      base::BindOnce(&GettingStartedHelper::OnURLResponse,
                     base::Unretained(this), *page_type),
      kMaxDownloadBytes);
}

void GettingStartedHelper::OnURLResponse(EducationPageType page_type,
                                         std::optional<std::string> body) {
  if (URLLoadedWithSuccess() && body) {
    RunCallbacks(GetEducationPageBrowserURL(page_type));
  } else {
    RunCallbacks(std::nullopt);
  }
}
...
void GettingStartedHelper::RunCallbacks(std::optional<GURL> webui_url) {
  url_loader_.reset();
  std::vector callbacks = std::move(url_callbacks_);
